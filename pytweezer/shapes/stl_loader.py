...
class STLLoader(TriangularMesh):
...
#      % see tplicenses/stl_EricJohnson.txt for licensing information.
    ...
    #    assert(nargin == 1, 'Filename not supplied');
        assert isinstance(filename, str), "Incorrect variable type for filename, string type is expected."
...
#         error(lasterror); %#ok
#      end
        stl_object = stl.mesh.Mesh.from_file(filename)
        verts = np.around(np.unique(stl_object.vectors.reshape([int(stl_object.vectors.size/3),3]), axis=0),2).T
        self._verts = verts
...
'''
xfaces      (TriangularMesh) 3xN matrix of vertex indices describing faces
