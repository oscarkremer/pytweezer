...
@cc.export(
    "compute_y_exp_minus_and_plus_n",
    "Tuple((c16[:,:], c16[:], c16[:]))(i8[:], i8[:,:], f8[:], f8[:,:])",
)
...
@cc.export(
    "compute_y_theta_phi",
    "(c16[:,:], c16[:,:], c16[:], c16[:], i8, i8[:], i8[:], i8[:,:], f8[:])",
)
def compute_y_theta_phi(Y, Y2, expplus, expminus, n, m, mi, mv, theta):
    ...
    ymminus[1:, :] = Y[:-1, :]
    Ytheta = (
        np.sqrt((n - mv + 1) * (n + mv)) / 2 * expplus * ymminus
        - np.sqrt((n - mv) * (n + mv + 1)) / 2 * expminus * ymplus
    )
    ymplus = Y2[2:, :]
    ymminus = Y2[:-2, :]
    Yphi = (
        ...
        * np.sqrt((2 * n + 1) / (2 * n + 3))
        * (
            np.sqrt((n + mv + 1) * (n + mv + 2)) * expminus * ymplus
            + np.sqrt((n - mv + 1) * (n - mv + 2)) * expplus * ymminus
        )
    )
    Y = Y[n + mi, :]
