def function tests = testTmatrixDda
...
def setupOnce(testCase):
    addpath('../')

def testSphere(testCase)
    abs_tol = 1.0e-4;
    rel_tol = 3.0e-2;
...
def testDipoleSphere(testCase):
    xyz = [0;0;0]
    nrel = 1.2
    radius = 0.01
    d = (4*pi/3).^(1/3) .* radius
    Tdda = ott.TmatrixDda(xyz, 'index_relative', nrel, 'spacing', d, 'Nmax', 1, 'polarizability', 'CM', 'z_rotational_symmetry', 1, 'z_mirror_symmetry', false)
    shape = ott.shapes.Sphere(radius);
    Tmie = ott.TmatrixMie.simple(shape, 'index_relative', nrel, 'Nmax', 1);
    Tdda_diag = diag(Tdda.data);
    Tmie_diag = full(diag(Tmie.data));
    testCase.verifyEqual(Tdda_diag(4:end), Tmie_diag(4:end), 'RelTol', 0.001, 'Upper Mie coefficeints not equal');
    testCase.verifyEqual(Tdda.data, full(Tmie.data), 'AbsTol', 1e-6, 'Upper Mie coefficeints not equal');


def testSingleDipole(testCase):
    Nmax = 2;
...
def testTooManyDipoles(testCase)
    wavelength_0 = 1
    ...
    testCase.verifyError(@() ott.TmatrixDda.simple(shape, 'index_medium', n_medium,
                                                   'index_particle', n_particle, 'wavelength0', wavelength_0),
                         'OTT:TmatrixDda:too_many_dipoles');

def testHomogeneousMaterialSizes(testCase)
    """
    ...
    nrel = 1.2
    Tdda = ott.TmatrixDda.simple(shape, 'index_relative', nrel, 'spacing', 1/20);

    nrel = [1.2; 1.3; 1.4]
    spacing = 1/20
    Tdda1 = ott.TmatrixDda.simple(shape, 'index_relative', nrel, 'spacing', spacing);

    nrel = [1.2; 1.3; 1.4]
    pol = ott.utils.polarizability.LDR(spacing, nrel)
    pol = diag(pol)
    Tdda2 = ott.TmatrixDda.simple(shape, 'polarizability', pol, 'spacing', spacing, 'index_relative', nrel)
    testCase.verifyEqual(Tdda2.data, Tdda1.data, 'AbsTol', 1e-16, 'Polarizability and index_relative not equivilant');


def testInhomogeneousPolarisability(testCase):
    shape = ott.shapes.Sphere(0.1)
    n_medium = 1.1
    nrel = [1.2; 1.3; 1.4]
    spacing = 1/20
    Tdda1 = ott.TmatrixDda.simple(shape, 'index_relative', nrel, 'spacing', spacing, 'index_medium', n_medium, 'wavelength0', 1.0)
    Ndipoles = 32
    nrel = [1.2; 1.3; 1.4]
    pol = ott.utils.polarizability.LDR(spacing.*n_medium, nrel)
    pol = repmat(diag(pol), [1, Ndipoles])
    nrel = repmat(diag(nrel), [1, Ndipoles])
    Tdda2 = ott.TmatrixDda.simple(shape, 'polarizability', pol, 'spacing', spacing, 'index_relative', nrel, 'index_medium', n_medium, 'wavelength0', 1.0)

    testCase.verifyEqual(Tdda2.data, Tdda1.data, 'AbsTol', 1e-15, 'Polarizability and index_relative not equivilant')

def testSphereRotSym(testCase)
    abs_tol = 1e-5
    rel_tol = 22.2e-2
    nrel = 1.2
    shape = ott.shapes.Sphere(0.1)
    ...
    Tdda = ott.TmatrixDda(voxels, 'index_relative', nrel, 
                          'Nmax', Nmax, 'spacing', spacing, 
...
def testSphereRotMirSym(testCase)
    # Create a sphere T-matrix with z-axis rotational symmetry
...
def testSphere3Modes(testCase):
    ...
    #WHATFUCK IS THIS NOTATION????????????????????????????????
    
    ###### FIX THIS THING HERE BELOW
    #Tdda = ott.TmatrixDda.simple(shape, 'index_relative', nrel, 'spacing', spacing, 'index_medium', 1.0, 'wavelength0', wavelength0, 'modes', (1:3).');
    Tmie = ott.TmatrixMie.simple(shape, 'index_relative', nrel, 'index_medium', 1.0, 'wavelength0', wavelength0);

    testCase.verifyEqual(Tdda.Nmax, Tmie.Nmax, 'Nmax does not match Mie T-matrix');
    ...
    testCase.verifyEqual(full(ndiag_dda), full(ndiag_mie),  'AbsTol', abs_tol,
        'T-matrix abstol failed');
...
def testDipoleFarfield(testCase):
...
def testFieldModes(testCase):
    ...
    testCase.verifyEqual(farfield, nearfield./P, 'AbsTol', 1.0e-3, 'near/far limit');

    # Test against some actual values
    total_modes = ott.utils.combined_index(Nmax, Nmax);
    ab = randn(2*total_modes, 1);
    beam = ott.Bsc(ab(1:end/2), ab(end/2+1:end), 'outgoing', 'incident', 'k_medium', k);
...
def testDipoleNearfield(testCase)
...
def testOneDipoleDirect(testCase)
...
def testNearfieldMatrixTotal(testCase):
    Ndipoles = 30;
...
def testTwoDipolesDirect(testCase):
    ...
    testCase.verifyEqual(real(E1*exp(-1i*offset*k)), real(E2), 'RelTol', 0.1, 'AbsTol', 1.0e-3);


def testInteractionA(testCase):
