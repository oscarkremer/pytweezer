...
class TriangularMesh(Shape, ABC):

    def __init__(self):
        # faces 3xN Matrix of vertex locations

        # verts 3xN matrix of vertex indices describing faces
        # protected variables
        pass

    def get_max_radius(self, shape):
        #Calculate the maximum distance from the shape origin
        return np.sqrt(np.sum(np.power(shape.verts,2), 0)).max()
...
#        return sum(volume) #;%divergence theorem


#    def inside(shape, radius, theta, phi):
...
#    def writeWavefrontObj(self, shape, filename):
